...
class IEGMN(nn.Module):
    ...
    def forward(self, lig_graph, rec_graph, geometry_graph, complex_names, epoch):
        ...
        if SAVE_LIG_RESULT:
            ...
            lig_path = f'data/PDBBind/{name}/{name}_ligand.pdb'
        ...
        for i, layer in enumerate(self.iegmn_layers):
            ...
            if SAVE_LIG_RESULT:
                # save layer
                lig_pdb.df['HETATM'][['x_coord', 'y_coord', 'z_coord']] = coords_lig.cpu().numpy()
...
class EquiBind(BaseModel):

    def __init__(self, device='cuda', debug=False, use_evolved_lig=False, evolve_only=False, loss_cfg=dict(), **kwargs):
        ...
        self.loss_func = BindingLoss(**loss_cfg)
    ...
    def forward(self, lig_graph, rec_graph, lig_coords, rec_coords, lig_pocket_coords, rec_pocket_coords, geometry_graph, complex_name, mode='loss', **kwargs):
        message_hub = MessageHub.get_current_instance()
        epoch = message_hub.get_info('epoch')
        lig_graphs = lig_graph.to('cuda')
        rec_graphs = rec_graph.to('cuda')
        geometry_graphs = geometry_graph.to('cuda')

        ligs_coords_pred, ligs_keypts, recs_keypts, rotations, translations, geom_reg_loss = self.forward_bind(
            lig_graphs, rec_graphs, geometry_graphs,
            complex_names=complex_name,
            epoch=epoch
        )
        if mode == 'loss':
            losses = self.loss_func(
                lig_coords, rec_coords, ligs_coords_pred, lig_pocket_coords,
                rec_pocket_coords, ligs_keypts, recs_keypts, rotations, translations, geom_reg_loss,
                self.device)
            return losses
    ...
    def forward_bind(self, lig_graph, rec_graph, geometry_graph=None, complex_names=None, epoch=0):
