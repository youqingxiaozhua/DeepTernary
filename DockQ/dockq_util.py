...
def cal_dockq(model_path, native_path):
    capri_peptide = False
    useCA = False
    capri_peptide = False
    skip_check = False
...
#    print native_chains
    if((len(model_chains) > 2 or len(native_chains) > 2) and
       (model_chain1 == None and native_chain1 == None)):
        ...
        sys.exit()
    if not skip_check and (len(model_chains) < 2 or len(native_chains)< 2):
    ...
    if len(model_chains) > 2 or len(native_chains)> 2:
        ...
        if(model_chain1 != None):
            group1=model_chain1
            nat_group1=group1
            if(model_chain2 != None):
                group2=model_chain2
            else:
        ...
        if(native_chain1 != None):
            nat_group1=native_chain1
            if(native_chain2 != None):
                nat_group2=native_chain2
            else:
                #will use the complement from group1
                nat_group2=[]
...
#        info=calc_DockQ(model,native,use_CA_only=)
    ...
    print(f'fnat:\t{fnat:.4f}, IRMSD:\t{irms:.4f}, LRMSD:\t{Lrms:.4f}, DockQ:\t{DockQ:.4f}')
